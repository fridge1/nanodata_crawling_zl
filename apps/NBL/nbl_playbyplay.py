...
class pbp_box(object):
    ...
    def pbp_box_live(self,data_queue,match_time):
        while True:
            headers = {
                        'user_agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.70 Safari/537.36',
                    }
            url = 'https://www.fibalivestats.com/data/1307440/data.json'
            logger.info(url)
            pbp_res = requests.get(url,headers=headers)
            if int(match_time) >= int(time.time()) and pbp_res.status_code != 200:
                logger.info('比赛未开赛.... %s' % str(1307440))
                time.sleep(10)
            else:
                ...
                match_data_boxscore = {'match': {'id': int(1307440),
                                                 'basketball_items': {
                                                     'player_stat': {
                ...
                logger.info('球员技术统计推送完成... %s' % str(1307440))
                match_data_playbyplay = {'match': {'id': int(1307440),
                                                   'basketball_items': {
                                                       'incident': {
                                                           'period': period_total,
                ...
                logger.info('球员技术文字直播推送完成... %s' % str(1307440))
    ...
    def get_match_id(self,data_queue):
        try:
            ...
            for id_time in match_dict['data']:
                ...
                pbp_box().pbp_box_live(data_queue,match_time)
        except:
