...
def get_score(match_id):
    ...
    url = 'https://live.euroleague.net/api/PlayByPlay?gamecode=%s&seasoncode=E2019&disp=' % str(match_id)[4:]
    res = requests.get(url,headers=headers)
    if res.text == '':
        logger.info('比赛未开赛。。。')
        return 0
    else:
        home_scores,away_scores,home_score,away_score = scores(json.loads(res.text))
        status_id,match_time = status(json.loads(res.text),home_score,away_score)
        data = {
            'sport_id': 2,
            'site': 'eur',
            'matches': {
...
def status(text,home_score,away_score):
    playbyplay = {}
    keys = list(text.keys())
    period = 0
    for key in keys[6:]:
