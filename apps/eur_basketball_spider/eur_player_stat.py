from apps.eur_basketball_spider.tools import seasons,get_team_id,get_player_id_upsert,get_player_position_upsert
...
season_id = 2019
gamecode = 60
box_api_url = 'https://live.euroleague.net/api/Boxscore?gamecode=60&seasoncode=E2019&disp='
...
if box_api_res.text == '':
    ...
    for index in range(2):
        for player_box in box_api_dict['Stats'][index]['PlayersStats']:
            sport_id = 2
            match_id = int(str(seasons[str(season_id)+'-'+str(season_id+1)]) + '0000') + int(gamecode)
