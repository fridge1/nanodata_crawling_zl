...
class GetPlayerTeamCoachInfo(object):
    def __init__(self):
        self.get_team_id = get_team_id()

    def get_coach_info(self,season_id):
        headers = {
    ...
    def get_team_info(self,season_id):
        headers = {
            'user_agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.70 Safari/537.36',
        }
        start_url = 'https://www.euroleague.net'
    ...
    def get_player_info(self,season_id):
        headers = {
            'user_agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.70 Safari/537.36',
        }
        ...
        for team_info in team_list:
            ...
            player_urls = player_list_tree.xpath(
                '//div[@class="wp-module"]/div[@class="item player"]/div[@class="img"]/a/@href')
            for player_url in player_urls:
                ...
                if 'Height' in player_tree.xpath('//div[@class="summary-second"]/span[1]/text()')[0].split(':')[0]:
                    ...
                    player['nationality'] = \
                        player_tree.xpath('//div[@class="summary-second"]/span[last()]/text()')[0].split(':')[-1]
                try:
                    player['team_id'] = self.get_team_id[team_key.lower()]
                except:
                    player['team_id'] = 0
                try:
