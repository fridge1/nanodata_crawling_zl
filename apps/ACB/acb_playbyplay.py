    ...
    def pbp_box_live(self, data_queue, match_id):
        while True:
            try:
                ...
                logger.info(url)
                pbp_res = requests.get(url, headers=headers,timeout=30)
                if pbp_res.status_code != 200:
                    ...
                    for key in keys:
                        try:
                            ...
                            BkMatchTeamStats['two_points_goals'] = safe_get(pbp_dict,'tm.%s.tot_sTwoPointersMade' % key)
                            BkMatchTeamStats['two_points_total'] = safe_get(pbp_dict,'tm.%s.tot_sTwoPointersAttempted' % key)
                            BkMatchTeamStats['three_point_goals'] = safe_get(pbp_dict,'tm.%s.tot_sThreePointersMade' % key)
                            BkMatchTeamStats['three_point_field'] = safe_get(pbp_dict,'tm.%s.tot_sThreePointersAttempted' % key)
                            BkMatchTeamStats['free_throw_goals'] = safe_get(pbp_dict,'tm.%s.tot_sFreeThrowsMade' % key)
                            BkMatchTeamStats['free_throw_field'] = safe_get(pbp_dict,'tm.%s.tot_sFreeThrowsAttempted' % key)
                            ...
                            BkMatchTeamStats['personal_fouls'] = safe_get(pbp_dict,'tm.%s.tot_sFoulsPersonal' % key)
                            BkMatchTeamStats['point'] = safe_get(pbp_dict,'tm.%s.tot_sPoints' % key)
